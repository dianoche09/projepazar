                ...
                <div key={l.id} className="flex flex-wrap items-center justify-between gap-4 p-4 text-sm hover:bg-paper/50">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray">
                      Talep Edilen: <span className="font-medium text-ink">{(l.proje as { ad?: string } | null)?.ad}</span> · Daire {(l.birim as { daire_no?: string } | null)?.daire_no || "—"}
                    </p>
                  </div>
                  <div className="flex items-center gap-4 text-right">
                    <div className="text-xs">
                      <p className="font-medium text-ink">Emlakçı: {(l.atanan as { ad?: string } | null)?.ad || "—"}</p>
                      <p className="text-gray font-mono">{(l.atanan as { telefon?: string } | null)?.telefon || ""}</p>
                    </div>
