...
export default async function HesapBekliyor() {
  ...
  return (
    <main className="flex min-h-screen flex-1 items-center justify-center bg-paper px-6 py-16 relative text-slate-900">
      ...
      <div className="glass-card rounded-2xl p-8 max-w-md text-center shadow-cardlg relative z-10">
        <span className="mx-auto mb-6 flex size-16 items-center justify-center rounded-full bg-blue-50 border border-blue-200/60 relative">
        ...
        <h1 className="font-display text-2xl font-bold text-slate-950 tracking-tight">{m.baslik}</h1>
        <p className="mt-3 text-sm text-slate-500 leading-relaxed font-medium">{m.metin}</p>
        
        {profil?.ad && (
          <p className="mt-4 font-mono text-xs text-teal border border-teal/20 bg-blue-50/50 rounded-lg px-3 py-1.5 inline-block font-semibold">
            {profil.ad}
          </p>
        )}
        ...
        <div className="mt-8 flex flex-col gap-3">
          ...
          <form action={cikisYap} className="w-full">
            <button className="w-full rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-800 transition-all duration-300 hover:bg-slate-50">
              Çıkış yap
            </button>
          </form>
        </div>
